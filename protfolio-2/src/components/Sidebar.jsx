...
export default function Sidebar({data}) {
  ...
  return (
    <div className={`${isOpen ? 'w-64' : 'w-10 md:w-14'} sm:mr-1 md:p-1 text-white flex flex-col justify-center items-center transition-all duration-300`}>
                ...
                data.map((item, index) => {
                    ...
                    return(
                        <Link
                            ...
                            key={index}>
                            {
                            isOpen ? <p>{index + 1}. {item.title}</p> : <p className='text-center px-3'>{index + 1}</p>
                            }
                        </Link>
                    )
